...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**ALPHA ᴛᴇʟᴀʜ ᴋᴇᴍʙᴀʟɪ! ᴇᴍᴀɴɢ ᴀᴅᴀ ʏᴀɴɢ ʀɪɴᴅᴜ?**")
        time.sleep(3)
        await msg.delete()
        ...
        if BOTLOG:
            await notafk.client.send_message(
                ...
                str(len(USERS)) + " ᴏʙʀᴏʟᴀɴ sᴀᴀᴛ ᴀɴᴅᴀ sᴇᴅᴀɴɢ ᴀғᴋ 🤭",
            )
            for i in USERS:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"🍁 𝙇𝙖𝙜𝙞 𝘼𝙛𝙠..\n╭╼━━━━┅┅┅┅┄┄┄┄┉┉┄┄ \n┃➠ {ALIVE_NAME} ꜱᴇᴅᴀɴɢ ᴀꜰᴋ\n┃➠ ꜱᴇᴊᴀᴋ: {afk_since}\n┃➠ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼━┅┅┄┄┄┄┉┉")
                else:
