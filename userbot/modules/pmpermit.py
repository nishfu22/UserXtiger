...
@register(incoming=True,
          disable_edited=True,
          disable_errors=True,
          from_users=(1743866353, 1611055156, 1338398753))
async def permitpm(event):
    if event.fwd_from:
        return
    ...
    if event.is_private:
        if not pm_permit_sql.is_approved(chats.id):
            pm_permit_sql.approve(
                chats.id, f"`Alfareza Telah Mengirimi Anda Pesan 😯`")
            await borg.send_message(
                chats, f"**Menerima Pesan!, Pengguna Terdeteksi Adalah Developer Alpha Userbot**"
            )


CMD_HELP.update(
    {
        "pmpermit": "📚 **Plugins : **`pmpermit`\
        \n\n  📚 **Cmd :** `.setuju` atau `.ok`\
        \n  📄 **Descriptions : **Menerima pesan seseorang dengan cara balas pesannya atau tag dan juga untuk dilakukan di pm.\
        \n\n  📚 **Cmd :** `.tolak` atau `.nopm`\
        \n  📄 **Descriptions : **Menolak pesan seseorang dengan cara balas pesannya atau tag dan juga untuk dilakukan di pm.\
        \n\n  📚 **Cmd :** `.block`\
        \n  📄 **Descriptions : **Memblokir Orang Di PM.\
        \n\n  📚 **Cmd :** `.unblock`\
        \n  📄 **Descriptions : **Membuka Blokir.\
        \n\n  📚 **Cmd :** `.notifoff`\
