...
async def _(fry):
    await fry.edit("`Tiger Mengeluarkan Kekuatan Telegram...🔥`")
    level = fry.pattern_match.group(2)
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`Mohon Balas Di Sticker Tiger`")
        return
    ...
    async with fry.client.conversation(chat) as conv:
        try:
            ...
            await fry.reply("`Alpha Mohon Unblock` @image_deepfrybot`...`")
            return
        if response.text.startswith("Forward"):
            await fry.edit("`Alpha Mohon Matikan Setelan Forward Privasi...`")
        else:
...
async def _(fry):
    ...
    if fry.fwd_from:
        return
    if not fry.reply_to_msg_id:
        await fry.edit("`Mohon Balas Di Sticker Alpha`")
        return
    reply_message = await fry.get_reply_message()
    if not reply_message.media:
        await fry.edit("`Mohon Balas Di Sticker Alpha`")
    ...
    async with fry.client.conversation(chat) as conv:
        try:
            ...
            return
        if response.text.startswith("Forward"):
            await fry.edit("`Alpha Mohon Matikan Setelan Privasi Forward...`")
        else:
            downloaded_file_name = await fry.client.download_media(
                response.media,
                TEMP_DOWNLOAD_DIRECTORY
