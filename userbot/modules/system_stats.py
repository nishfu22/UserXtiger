...
from userbot import ALIVE_LOGO, ALIVE_NAME, ALIVE_TEKS_KUSTOM, CMD_HELP, StartTime, bot
...
@register(outgoing=True, pattern=f"^\\.spc")
async def psu(event):
...
@register(outgoing=True, pattern=f"^\\.sysd$")
...
@register(outgoing=True, pattern=f"^\\.pip(?: |$)(.*)")
...
@register(outgoing=True, pattern=f"^\\.(?:alpha|alphaon)\\s?(.)?")
...
@ register(outgoing=True, pattern=f"^\\.(?:xalive|xon)\\s?(.)?")
async def amireallyalive(alive):
...
@ register(outgoing=True, pattern=f"^\\.(?:alive|on)\\s?(.)?")
async def amireallyalive(alive):
    user = await bot.get_me()
    await get_readable_time((time.time() - StartTime))
    await alive.edit("①")
...
@ register(outgoing=True, pattern=f"^\\.aliveu")
...
@ register(outgoing=True, pattern=f"^\\.resetalive$")
